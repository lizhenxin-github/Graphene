...
#include <vector>
#include <assert.h>
#include <libhiergraph.h>

using std::vector;

degree_t query_vert_nebrs(HierGraph* hiergraph, vid_t vid){
    degree_t nebr_count, local_degree, deg_sum = 0;
    vid_t* local_adjlist;

    // query out-neighbors
    if(hiergraph->get_out_graph()){
        nebr_count = hiergraph->get_out_degree(vid);
        // std::cout << vid << ": outd = " << nebr_count << std::endl;
        if (0 == nebr_count) return deg_sum;
        local_adjlist = new vid_t[nebr_count];
        local_degree  = hiergraph->get_out_nebrs(vid, local_adjlist);
    ...
    // query in-neighbors
    if(hiergraph->get_out_graph()){
        nebr_count = hiergraph->get_in_degree(vid);
        // std::cout << vid << ": ind = " << nebr_count << std::endl;
        if (0 == nebr_count) return deg_sum;
        local_adjlist = new vid_t[nebr_count];
        local_degree  = hiergraph->get_in_nebrs(vid, local_adjlist);
        assert(local_degree  == nebr_count);
        deg_sum += nebr_count;
        delete [] local_adjlist;
    }
...
index_t test_1hop(HierGraph* hiergraph, vector<vid_t> query_verts) {
    // vid_t v_count = hiergraph->get_vcount();
    // if (query_count == 0) query_count = v_count / 10;
    // vector<vid_t> query_verts = root_generator(hiergraph, query_count);
    ...
    #pragma omp parallel
    {
        ...
        for(auto it = query_verts.begin(); it != query_verts.end(); ++it){
            vid_t vid = *it;
            deg_sum += query_vert_nebrs(hiergraph, vid);
        }
    }
...
index_t test_1hop_numa(HierGraph* hiergraph, vector< vector<vid_t> > query_verts) {
    // vid_t v_count = hiergraph->get_vcount();
    // if (query_count == 0) query_count = v_count / 10;

    // vector< vector<vid_t> > query_verts = root_generator_numa(hiergraph, query_count);
	// double start = mywtime();
