...
inline bool is_bfs_active(index_t vert_id,
						  sa_t criterion,
						  sa_t *sa, sa_t *prior)
{
...
inline bool is_wcc_active(index_t vert_id,
						  sa_t criterion,
...
#pragma omp barrier
		...
		if ((tid & 1) == 0)
		{
			...
			{
				printf("Degree[%u]=%ld\n", root,
					   it->beg_pos_ptr[root + 1 - it->row_ranger_beg] - it->beg_pos_ptr[root - it->row_ranger_beg]);
...
#pragma omp barrier
		if (!tid)
			printf("Largest WCC is detected\n");
		for (vertex_t vert = beg_1d; vert < end_1d; vert++)
			if (sa[vert] != INFTY)
				sa[vert] = 0;
		...
		// WCC starts
		if (!tid)
			printf("Entering label propagation ...\n");
#pragma omp barrier
		while (true)
		{
